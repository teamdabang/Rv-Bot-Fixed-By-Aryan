...
from cryptography.fernet import Fernet as FcukYou
...
class Cdm:
    def __init__(self, device_md5):
        self.logger = logging.getLogger(__name__)
        self.sessions = {}
        self.__device_md5 = FcukYou(device_md5[4:(-6)] + '=')
    ...
    def open_session(self, init_data_b64, device, raw_init_data=None, offline=False):
        ...
        if device.session_id_type == 'android':
            ...
            if device.session_id_type == 'chrome':
                ...
                return 1
        if raw_init_data:
            if isinstance(raw_init_data, (bytes, bytearray)):
    ...
    def get_license_request(self, session_id):
        ...
        with open(session.device_config.device_client_id_blob_filename, 'rb') as f:
            try:
                cid_bytes = client_id.ParseFromString(self.__device_md5.decrypt(f.read()))
            except DecodeError:
        ...
        if not session.privacy_mode or session.device_config.vmp:
...
#        else:  # inserted
        ...
        if session.device_config.private_key_available:
            key = RSA.importKey(self.__device_md5.decrypt(open(session.device_config.device_private_key_filename, 'rb').read()).decode())
        ...
        self.logger.debug('license request b64: {}'.format('license request b64: {}'))
    ...
    def provide_license(self, session_id, license_b64):
        ...
        try:
            ...
            self.logger.error(f'unable to parse license - check protobufs: {e}')
            return 1
